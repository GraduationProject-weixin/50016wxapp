...
# 50016wxapp微信的设备故障报修管理系统设计与实现

![picture](https://raw.githubusercontent.com/GraduationProject-springboot/.github/main/img/wx.png)

### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1BPtKekEv3?p=19)
...
# 3系统分析
## 3.1可行性分析
通过对本基于微信的设备故障报修管理系统实行的目的初步调查和分析，提出可行性方案并对其一一进行论证。我们在这里主要从技术可行性、经济可行性、操作可行性等方面进行分析。
### 3.1.1技术可行性
本基于微信的设备故障报修管理系统后端采用SSM框架进行开发，是一种成熟的框架，开发有很多教程的，微信开发也有很多教程，并且网络上很多免费的教程。  

因此，基于微信的设备故障报修管理系统在开发技术上具有很高可行性，且开发人员掌握了一定的开发技术，所以此系统的开发技术具有可行性。
### 3.1.2经济可行性
本基于微信的设备故障报修管理系统采用的软件都是开源的，这样能够削减很多的精力和资源，降低开发成本。同时对计算机的配置要求也极低，即使是淘汰下来的计算机也能够满足需要，因此，本系统在经济上是完全具有可行性的，所以在经济上是十分可行的。
### 3.1.3操作可行性
本基于微信的设备故障报修管理系统的界面简单易操作，用户只要平时有在用过电脑，都能进行访问和操作。本系统具有易操作、易管理、交互性好的特点，在操作上是非常简单的，因此在操作上具有很高的可行性。

综上所述，此系统开发目标已明确，在技术、经济和操作方面都具有很高的可行性，并且投入少、功能完善、管理方便，因此系统的开发是完全可行的。
## 3.2系统性能分析
### 3.2.1 系统安全性
此基于微信的设备故障报修管理系统要严格控制管理权限，具体要求如下：

（1）要想对基于微信的设备故障报修管理系统进行管理，首先要依靠用户名和密码在系统中登陆，无权限的用户不可以通过任何方式登录系统和对系统的任何信息和数据进行查看，这样可以保证系统的安全可靠性和准确性。
...
图4.14 维修员实体属性图
### 4.3.2数据库设计表
基于微信的设备故障报修管理系统需要后台数据库，下面介绍数据库中的各个表的详细信息：
...
|1|Id|Int|id|否|
|2|addtime|Date|创建时间|是|
|3|shiyanshihao|String|实验室号|是|
|4|shiyanshimingcheng|String|实验室名称|是|
|5|tupian|String|图片|是|
|6|shebeimingcheng|String|设备名称|是|
|7|baoxiuneirong|String|报修内容|是|
|8|baoxiuriqi|date|报修日期|是|
...
|4|shiyanshimingcheng|String|实验室名称|是|
|5|tupian|String|图片|是|
|6|shiyanshixiangqing|String|实验室详情|是|
|7|shebeimingcheng|String|设备名称|是|
|8|shiyongzhuangtai|String|使用状态|是|
5维修结果表
...
|1|Id|Int|id|否|
|2|addtime|Date|创建时间|是|
|3|mingcheng|String|名称|是|
|4|shiyanshimingcheng|String|实验室名称|是|
|5|shiyanshihao|String|实验室号|是|
|6|shebeimingcheng|String|设备名称|是|
|7|zhanghao|String|账号|是|
|8|xingming|String|姓名|是|
|9|tupian|String|图片|是|
|10|weixiuneirong|String|维修内容|是|
|11|riqi|date|日期|是|
...
|5|sunhuaiyuanyin|String|损坏原因|是|
|6|weixiufangfa|String|维修方法|是|
...
|5|tupian|String|图片|是|
|6|shebeimingcheng|String|设备名称|是|
|7|sunhuaiyuanyin|String|损坏原因|是|
|8|weixiuqingkuang|String|维修情况|是|
|9|riqi|date|日期|是|
|10|shifouchuli|String|是否处理|是|
